...
- Added `docs/BT_GYRO_INVESTIGATION.md` to record the Genshin BT-source gyro investigation. BT source → DS5 USB target now uses fixed-rate seq-only UHID repeat by default at 1000Hz; repeat frames advance `raw[7]` but keep the sensor timestamp unchanged. `DSEUHID_BT_DS5_USB_REPEAT_HZ` overrides the rate, and `DSEUHID_BT_DS5_USB_REPEAT_MODE=passthrough` restores one UHID input per physical BT input.
