...
mod tests {
    ...
    #[test]
    fn canonical_key_names_cover_registered_keycodes_in_order() {
        let resolved: Vec<u16> = KEYCODE_NAMES
            .iter()
            .map(|name| resolve_keycode(name).expect("canonical key name must resolve"))
            .collect();
        assert_eq!(resolved, ALL_KEYCODES);
    }
}
